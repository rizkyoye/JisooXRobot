...
async def hmm(_, message):
    ...
    if len(message.command) != 2:
        ...
        await message.reply_text(
            ...
            "Saya hanya tahu `/chatbot on` dan /chatbot `off saja`"
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        lel = await edit_or_reply(message, "`Tunggu...`")
        ...
        if not chat_id in en_chats:
            ...
            await message.reply_text("Obrolan AI bahasa Inggris Diaktifkan!")

            return

        await message.reply_text("Obrolan AI Sudah Dinonaktifkan.")
