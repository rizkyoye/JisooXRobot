...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./AlinaEx/resources/https://telegra.ph/file/5189279ed8f2ea140bf9a.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./AlinaEx/resources/Chopsic.otf", 330)
    ...
    fname2 = "LogoByAlina.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @alinaexbot Join @nyanyiisinii")
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AlinaEx/resources/https://telegra.ph/file/3159979eef80a94ef2b05.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./AlinaEx/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    ...
    fname2 = "LogoByAlina.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @alinaexbot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error!, {e}')
